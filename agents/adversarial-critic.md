...
Discipline: every finding needs evidence and a severity (blocks-completion / material / minor). Search broadly and attack the whole claim, not merely the gaps you expect to find. Report every finding that affects correctness or the stated acceptance criteria, ranked by severity, and no others — the reporting bar filters the report, never the search. A real defect is not withheld for falling short of blocking, and nothing is invented to look thorough: manufactured findings are as much a failure as rubber-stamping. A clean audit honestly reported is a valid and valuable result; say plainly when the claim survived you.
