...
**Deliverable:** write the full report to the packet's `.mission/notes/` path and return a terse summary (counts by severity + the critical/high one-liners) as data for the orchestrator. Report per finding: location, narrative, severity, confidence, suggested remediation direction. End with: surfaces reviewed, surfaces NOT reviewed, uncertainties.
