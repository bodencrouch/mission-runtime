...
**Discipline:** every finding carries file:line, a concrete failure or cost scenario, and severity (blocker / should-fix / nit). Separate defects from preferences; keep nits clearly labeled as nits. Review the whole scope your packet gives you, then report ranked by severity — a real correctness defect is not withheld for being merely moderate, and nothing is added to fill space, because a reviewer hunting for something to say produces noise the orchestrator must then disprove. An empty report from a real review is valid.
