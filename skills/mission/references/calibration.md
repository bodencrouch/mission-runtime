...
than any commission could. So the runtime holds no prior belief about which end it
...
Every commission, retry, and self-instruction the runtime writes fixes five things
...
not already say. Route detail enters a commission only through the ratchet below,
as a repair for something already observed.

Stating the reason is load-bearing, not decoration. A rule reaches the cases
the commission forgot to name only when the principle travels with it: "the
...
|---|---|
| A step-by-step route through work the commission already bounds | Caps a strong executor at the author's plan; adds nothing a weak one cannot get from the invariants |
...
Default to the lean form of every commission. Add scaffolding only as a repair for
...
1. The repair is minimal and aimed at the observed defect — one commission slot,
   not a template rewrite.
2. The repair names its observation: "delegate 4 refactored a module outside
   its scope; added an explicit do-not-touch list to write-capable commissions."
3. The repair is scoped to the agent role or the task class the evidence
   supports. A repair applied to every commission on one observation over-fits.
...
|---|---|
| Work outside the commission's scope — drive-by refactor, unrequested cleanup, abstraction for a hypothetical need | Name the fence explicitly and add "no changes not required by the objective" |
| Findings returned when a change was wanted, or a change made when findings were wanted | Put the deliverable type in the commission's objective and re-check the action boundary |
...
| Progress claimed that the run's tool results do not support | Add the grounding rule to the commission and re-verify the claim before integrating anything |
| The delegate stopped to ask something the contract already answers | Move the answer into the commission's context slot; a delegate's question is answered from the ledgers, never forwarded to the user |
...
| A previously failed approach was tried again | Carry the relevant attempts.md excerpt in the commission's context slot as do-not-repeat notes |

## Depth is dispatch configuration

Express task difficulty where the host accepts it — a depth or effort control,
a model selection, a task budget — and not as prose inside the commission. Prose
...
orchestrator does the work inline and keeps the commission as its own written
...
carries a short Calibration block: host capabilities probed, commission repairs in
